...
const maintenanceStartDateColumn = SortColumn<IProject>(
  'maintenance_start_date',
  'วันที่เริ่มต้น MA',
  'center'
);
const maintenanceEndDateColumn = SortColumn<IProject>(
...
export const createColumns = ({
  ...
  const baseColumns: ColumnDef<IProject>[] = [
    ...
    {
      ...maintenanceStartDateColumn,
      size: 110,
      cell: ({ row }) => {
        const { maintenance_start_date } = row.original;
        return (
          <div className="text-center">
            {maintenance_start_date
    ...
    {
      ...maintenanceEndDateColumn,
      size: 110,
      cell: ({ row }) => {
        const { maintenance_end_date } = row.original;
        return (
          <div className="text-center">
            {maintenance_end_date ? buddhistFormatDate(maintenance_end_date, 'dd mmm yyyy') : '-'}
          </div>
        );
      },
    },
